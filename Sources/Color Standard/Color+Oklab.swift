// Color+Oklab.swift
// Oklab conformance to Color.Protocol

import ISO_9899

// MARK: - Oklab Conformance to Color.Protocol

extension Color.Oklab: Color.`Protocol` {
    /// Converts Oklab to canonical color representation.
    ///
    /// The conversion:
    /// 1. Converts Oklab to linear sRGB via LMS intermediate
    /// 2. Converts linear sRGB to XYZ (D65 illuminant)
    ...
    /// Creates Oklab from canonical color representation.
    ///
    /// The conversion:
    /// 1. Converts XYZ to linear sRGB
    /// 2. Converts linear sRGB to Oklab via LMS intermediate
    ///
...
extension Color {
    ...
    ///
    /// Implementation follows Björn Ottosson's Oklab specification.
    ///
    /// - Parameter oklab: Oklab color to convert
    /// - Returns: Canonical color representation
    internal static func _fromOklab(_ oklab: Oklab) -> Color {
        // Oklab → LMS' (cone response). Variable names (l_, m_, s_) mirror
        // Björn Ottosson's published Oklab conversion notation verbatim, for
        // auditability against the reference formula.
        ...
        // LMS → linear sRGB (Oklab uses a different matrix than standard XYZ→sRGB)
    ...
    internal static func _toOklab(_ color: Color) -> Oklab {
        // Ensure D65 illuminant for Oklab
        let xyz =
        ...
        // LMS → LMS' (cube root). Variable names (l_, m_, s_, L) mirror
        // Björn Ottosson's published Oklab conversion notation verbatim, for
        // auditability against the reference formula.
        // swift-format-ignore: AlwaysUseLowerCamelCase
